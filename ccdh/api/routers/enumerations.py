...
async def get_enumerations_of_harmonized_attribute(system: str, entity: str, attribute: str, original: bool = False) -> Response:
    ...
    if original:
        ...
        for v in values:
            enum.permissible_values.append(PermissibleValue(text=v['pref_label']))
    else:
        concepts, values = mdr_graph.find_concept_references_and_permissible_values_of(system, entity, attribute)
        for value in values:
            value = dict(value)
            extensions = {'context': f"{value['system']}.{value['entity']}.{value['attribute']}"}
            pv = PermissibleValue(text=value['pref_label'])
