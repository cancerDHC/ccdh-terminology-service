...
@router.get('/{model}/{entity}/{attribute}',
            description='Given model, entity, and attribute get enumeration of concept references, including NCIT codes',
...
async def get_enumeration(model: str, entity: str, attribute: str, value_only: bool = False) -> Response:
    # if len(model_entity_attribute.split('.')) != 3:
    #     raise HTTPException(status_code=400, detail='Parameter format incorrect. Needs to be in format of <model>.<entity>.<attribute>.')
    # system, entity, attribute = model_entity_attribute.split('.')
    enum = EnumDefinition(name=f'{model}.{entity}.{attribute}', description=f'Autogenerated Enumeration for {model} {entity} {attribute}')
    enum.permissible_values = []
    if value_only:
        values = mdr_graph.find_permissible_values_of(model, entity, attribute)
        for v in values:
            enum.permissible_values.append(PermissibleValue(text=v['pref_label'], description=v['description']))
    else:
        concepts, values = mdr_graph.find_concept_references_and_permissible_values_of(model, entity, attribute)
        for v in values:
