...
async def get_enumeration(name: str, value_only: bool) -> Response:
    ...
    enum = EnumDefinition(name=name, description=f'Autogenerated Enumeration for {entity} {attribute}')
    enum.permissible_values = []
    if value_only:
        values = mdr_graph.find_permissible_values_of('CDM', entity, attribute)
        for v in values:
            enum.permissible_values.append(PermissibleValue(text=v['pref_label'], description=v['pref_label']))
    else:
        concepts, values = mdr_graph.find_concept_references_and_permissible_values_of('CDM', entity, attribute)
        for value in values:
